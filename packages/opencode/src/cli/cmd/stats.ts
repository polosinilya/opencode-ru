...
export const StatsCommand = effectCmd({
  command: "stats",
  describe: t("show token usage and cost statistics"),
  builder: (yargs) =>
    yargs
      .option("days", {
        describe: t("show stats for the last N days (default: all time)"),
        type: "number",
      })
      .option("tools", {
        describe: t("number of tools to show (default: all)"),
        type: "number",
      })
      .option("models", {
        describe: t("show model statistics (default: hidden). Pass a number to show top N, otherwise shows all"),
      })
      .option("project", {
        describe: t("filter by project (default: all projects, empty string: current project)"),
        type: "string",
      }),
...
const aggregateSessionStats = Effect.fn("Cli.stats.aggregate")(function* (
  ...
  if (filteredSessions.length > 1000) {
    console.log(t("Large dataset detected ({count} sessions). This may take a while...", { count: filteredSessions.length }))
  }
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number) {
  ...
  console.log("│                        ОБЗОР                         │")
  console.log("├────────────────────────────────────────────────────────┤")
  console.log(renderRow(t("Sessions"), stats.totalSessions.toLocaleString()))
  console.log(renderRow(t("Messages"), stats.totalMessages.toLocaleString()))
  console.log(renderRow(t("Days"), stats.days.toString()))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()

  // Cost & Tokens section
  console.log("┌────────────────────────────────────────────────────────┐")
  console.log("│                  СТОИМОСТЬ И ТОКЕНЫ                  │")
  console.log("├────────────────────────────────────────────────────────┤")
  const cost = isNaN(stats.totalCost) ? 0 : stats.totalCost
  const costPerDay = isNaN(stats.costPerDay) ? 0 : stats.costPerDay
  const tokensPerSession = isNaN(stats.tokensPerSession) ? 0 : stats.tokensPerSession
  console.log(renderRow(t("Total Cost"), `$${cost.toFixed(2)}`))
  console.log(renderRow(t("Avg Cost/Day"), `$${costPerDay.toFixed(2)}`))
  console.log(renderRow(t("Avg Tokens/Session"), formatNumber(Math.round(tokensPerSession))))
  const medianTokensPerSession = isNaN(stats.medianTokensPerSession) ? 0 : stats.medianTokensPerSession
  console.log(renderRow(t("Median Tokens/Session"), formatNumber(Math.round(medianTokensPerSession))))
  console.log(renderRow(t("Input"), formatNumber(stats.totalTokens.input)))
  console.log(renderRow(t("Output"), formatNumber(stats.totalTokens.output)))
  console.log(renderRow(t("Cache Read"), formatNumber(stats.totalTokens.cache.read)))
  console.log(renderRow(t("Cache Write"), formatNumber(stats.totalTokens.cache.write)))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()
  ...
  if (modelLimit !== undefined && Object.keys(stats.modelUsage).length > 0) {
    ...
    console.log("┌────────────────────────────────────────────────────────┐")
    console.log("│                   ИСПОЛЬЗОВАНИЕ МОДЕЛЕЙ               │")
    console.log("├────────────────────────────────────────────────────────┤")

    for (const [model, usage] of modelsToDisplay) {
      console.log(`│ ${model.padEnd(54)} │`)
      console.log(renderRow(t("  Messages"), usage.messages.toLocaleString()))
      console.log(renderRow(t("  Input Tokens"), formatNumber(usage.tokens.input)))
      console.log(renderRow(t("  Output Tokens"), formatNumber(usage.tokens.output)))
      console.log(renderRow(t("  Cache Read"), formatNumber(usage.tokens.cache.read)))
      console.log(renderRow(t("  Cache Write"), formatNumber(usage.tokens.cache.write)))
      console.log(renderRow(t("  Cost"), `$${usage.cost.toFixed(4)}`))
      console.log("├────────────────────────────────────────────────────────┤")
    }
  ...
  if (Object.keys(stats.toolUsage).length > 0) {
    ...
    console.log("┌────────────────────────────────────────────────────────┐")
    console.log("│               ИСПОЛЬЗОВАНИЕ ИНСТРУМЕНТОВ              │")
    console.log("├────────────────────────────────────────────────────────┤")
