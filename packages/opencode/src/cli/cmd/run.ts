...
async function toolError(part: ToolPart) {
  try {
    ...
    inline({
      icon: "✗",
      title: t("{title} failed", { title: next.title }),
    ...
    inline({
      icon: "✗",
      title: t("{title} failed", { title: part.tool }),
    })
  }
}
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: t("run opencode with a message"),
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("message", {
        describe: t("message to send"),
        type: "string",
        array: true,
        default: [],
      })
      .option("command", {
        describe: t("the command to run, use message for args"),
        type: "string",
      })
      .option("continue", {
        alias: ["c"],
      ...
      .option("fork", {
        describe: t("fork the session before continuing (requires --continue or --session)"),
        type: "boolean",
      })
      .option("share", {
        type: "boolean",
        describe: t("share the session"),
      })
      .option("model", {
        type: "string",
        alias: ["m"],
      ...
      .option("format", {
        ...
        default: "default",
        describe: t("format: default (formatted) or json (raw JSON events)"),
      })
      .option("file", {
        alias: ["f"],
        type: "string",
        array: true,
        describe: t("file(s) to attach to message"),
      })
      .option("title", {
        type: "string",
        describe: t("title for the session (uses truncated prompt if no value provided)"),
      })
      .option("attach", {
        type: "string",
        describe: t("attach to a running opencode server (e.g., http://localhost:4096)"),
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: t("basic auth password (defaults to OPENCODE_SERVER_PASSWORD)"),
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: t("basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'opencode')"),
      })
      .option("dir", {
        type: "string",
        describe: t("directory to run in, path on remote server if attaching"),
      })
      .option("port", {
        type: "number",
        describe: t("port for the local server (defaults to random port if no value provided)"),
      })
      .option("variant", {
        type: "string",
        describe: t("model variant (provider-specific reasoning effort, e.g., high, max, minimal)"),
      })
      .option("thinking", {
        type: "boolean",
        describe: t("show thinking blocks"),
      })
      .option("mini", {
        type: "boolean",
        hidden: true,
      ...
      .option("replay", {
        ...
        default: true,
        hidden: true,
        describe: t("replay interactive session history on resume and after resize (use --no-replay to disable)"),
      })
      .option("replay-limit", {
        type: "number",
        hidden: true,
        describe: t("cap visible interactive replay to the newest N messages"),
      })
      .option("interactive", {
        alias: ["i"],
        type: "boolean",
        describe: t("run in direct interactive split-footer mode"),
        default: false,
      })
      .option("auto", {
        type: "boolean",
      ...
      .option("demo", {
        ...
        default: false,
        hidden: true,
        describe: t("enable direct interactive demo slash commands; pass one as the message to run it immediately"),
      }),
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (interactive && args.command) {
        die(t("--mini cannot be used with --command"))
      }

      if (interactive && args._?.[0] !== "mini") {
        die(t("--mini must be used without the run subcommand"))
      }

      if (args.demo && !interactive) {
        die(t("--demo requires --mini"))
      }

      if (interactive && args.format === "json") {
        die(t("--mini cannot be used with --format json"))
      }

      if (args["replay-limit"] !== undefined && !interactive) {
        die(t("--replay-limit requires --mini"))
      }
      ...
      if (
        ...
        die(t("--replay-limit must be a positive integer"))
      }

      if (interactive && !process.stdout.isTTY) {
        die(t("--mini requires a TTY stdout"))
      }
      ...
      const directory = (() => {
        ...
        try {
          ...
          UI.error(t("Failed to change directory to {dir}", { dir: args.dir }))
          process.exit(1)
        }
      })()
      ...
      if (args.file) {
        ...
        for (const filePath of list) {
          ...
          if (!(await Filesystem.exists(resolvedPath))) {
            UI.error(t("File not found: {file}", { file: filePath }))
            process.exit(1)
          }
          ...
          if (args.attach && isDirectory) {
            UI.error(t("Cannot attach local directory without a shared filesystem: {file}", { file: filePath }))
            process.exit(1)
          }
          ...
          const content = await (async () => {
            ...
            try {
              ...
              if (!opened.isFile() || Number(opened.size) > ATTACH_FILE_MAX_BYTES) {
                UI.error(t("Cannot attach local file larger than 10 MiB or a special file: {file}", { file: filePath }))
                process.exit(1)
              }
      ...
      if (message.trim().length === 0 && !args.command && !interactive) {
        UI.error(t("You must provide a message or a command"))
        process.exit(1)
      }
      ...
      async function current(sdk: OpencodeClient): Promise<string> {
        ...
        UI.error(t("Failed to resolve remote directory"))
        process.exit(1)
      }
      ...
      async function localAgent() {
        ...
        if (!entry) {
          UI.println(
            ...
            t('agent "{name}" not found. Falling back to default agent', { name }),
          )
          return undefined
        }
        if (entry.mode === "subagent") {
          UI.println(
            UI.Style.TEXT_WARNING_BOLD + "!",
            UI.Style.TEXT_NORMAL,
            t('agent "{name}" is a subagent, not a primary agent. Falling back to default agent', { name }),
          )
          return undefined
        }
        return name
      ...
      async function attachAgent(sdk: OpencodeClient) {
        ...
        if (!modes) {
          UI.println(
            UI.Style.TEXT_WARNING_BOLD + "!",
            UI.Style.TEXT_NORMAL,
            t("failed to list agents from {url}. Falling back to default agent", { url: args.attach }),
          )
          return undefined
        }
            ...
            if (event.type === "permission.asked") {
              ...
              if (auto) {
                ...
                UI.println(
                  UI.Style.TEXT_WARNING_BOLD + "!",
                  UI.Style.TEXT_NORMAL +
                    t("permission requested: {permission} ({patterns}); auto-rejecting", {
...
export async function runMini(input: MiniCommandInput) {
  if (!RunCommand.handler) throw new Error(t("Mini command handler is unavailable"))
