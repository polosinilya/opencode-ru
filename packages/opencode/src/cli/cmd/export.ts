...
export const ExportCommand = effectCmd({
  ...
  describe: t("export session data as JSON"),
  builder: (yargs) =>
    yargs
      .positional("sessionID", {
        describe: t("session id to export"),
        type: "string",
      })
      .option("sanitize", {
        describe: t("redact sensitive transcript and file data"),
        type: "boolean",
      }),
...
const run = Effect.fn("Cli.export.body")(function* (args: { sessionID?: string; sanitize?: boolean }) {
  ...
  process.stderr.write(t("Exporting session: {id}\n", { id: sessionID ?? "latest" }))

  if (!sessionID) {
    UI.empty()
    prompts.intro(t("Export session"), { output: process.stderr })

    const sessions = yield* svc.list()

    if (sessions.length === 0) {
      prompts.log.error(t("No sessions found"), { output: process.stderr })
      prompts.outro(t("Done"), { output: process.stderr })
      return
    }
    ...
    const selectedSession = yield* Effect.promise(() =>
      prompts.autocomplete({
        message: t("Select session to export"),
        maxItems: 10,
    ...
    prompts.outro(t("Exporting session..."), { output: process.stderr })
  }
  ...
  }).pipe(Effect.catchCause(() => fail(t("Session not found: {id}", { id: sessionID! }))))
})
