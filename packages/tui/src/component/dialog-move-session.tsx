...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: t("Loading project directories..."), value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: t("No project directories found"), value: undefined }]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>{t("Deleting {location}", { location: item.location })}</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>{t("Press {hint} again to confirm", { hint: deleteHint() })}</span>
        ) : suffix ? (
          <>
        ...
        category: item.root.directory === current ? t("Current") : t("Other"),
        titleWidth,
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: t("Delete working copy?"),
          message: t("This working copy has file changes. Do you want to delete it anyway?"),
        })
        if (choice !== "yes") {
          reopen()
          return
        ...
        if (forced.error) {
          toast.show({
            variant: "error",
            title: t("Failed to delete project copy"),
          ...
          showError() ? (
            <box paddingLeft={4} paddingRight={4}>
              <text fg={theme.error} attributes={TextAttributes.BOLD}>
                {t("Could not load project directories")}
              </text>
