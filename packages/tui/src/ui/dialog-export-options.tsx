    ...
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box flexDirection="column">
        ...
        >
          ...
          <text fg={store.active === "thinking" ? theme.primary : theme.text}>{t("Include thinking")}</text>
        </box>
        <box
          flexDirection="row"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "toolDetails" ? theme.primary : theme.text}>{t("Include tool details")}</text>
        </box>
        <box
          flexDirection="row"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "openWithoutSaving" ? theme.primary : theme.text}>{t("Open without saving")}</text>
        </box>
      </box>
      <Show when={store.active !== "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t("Press")} <span style={{ fg: theme.text }}>{t("space")}</span> {t("to toggle,")}{" "}
          <span style={{ fg: theme.text }}>{t("return")}</span> {t("to confirm")}
        </text>
      </Show>
      <Show when={store.active === "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          {t("Press")} <span style={{ fg: theme.text }}>{t("return")}</span> {t("to confirm,")}{" "}
          <span style={{ fg: theme.text }}>{t("tab")}</span> {t("for options")}
        </text>
      </Show>
    </box>
  )
