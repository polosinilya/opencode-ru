...
const NO_MODELS_PARTS = parse(t(NO_MODELS_TIP))
...
function commandText(command: string, shortcut: string) {
  ...
  return `${shortcutText(command)} ${t("or")} ${shortcutText(shortcut)}`
}

function press(shortcut: string, text: string) {
  if (!shortcut) return undefined
  return `${t("Press")} ${shortcutText(shortcut)} ${t(text)}`
}
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const parts = createMemo(() => {
    const value = tip()
    if (typeof value === "string") return parse(t(value))
...
const TIPS: Tip[] = [
  ...
  t("Start a message with {highlight}!{/highlight} to run shell commands (e.g., {highlight}!ls -la{/highlight})"),
  ...
  t("Run {highlight}/share{/highlight} to create a public opencode.ai link"),
  t("Drag and drop images or PDFs into the terminal as context"),
  (shortcuts) => press(shortcuts.inputPaste(), "to paste images from your clipboard into the prompt"),
  (shortcuts) =>
    `${t("Use")} ${commandText("/editor", shortcuts.editorOpen())} ${t("to compose messages in your external editor")}`,
  t("Run {highlight}/init{/highlight} to auto-generate project rules based on your codebase"),
  (shortcuts) =>
    `${t("Use")} ${commandText("/models", shortcuts.modelList())} ${t("to switch between available AI models")}`,
  (shortcuts) =>
    `${t("Use")} ${commandText("/themes", shortcuts.themeList())} ${t("to switch between")} ${themeCount} ${t("built-in themes")}`,
  (shortcuts) =>
    `${t("Use")} ${commandText("/new", shortcuts.sessionNew())} ${t("to start a fresh conversation session")}`,
  (shortcuts) =>
    `${t("Use")} ${commandText("/sessions", shortcuts.sessionList())} ${t("to list, pin, and continue sessions")}`,
  (shortcuts) => press(shortcuts.sessionPinToggle(), "in the session list to pin one at the top"),
  (shortcuts) =>
    shortcuts.sessionQuickSwitch1() && shortcuts.sessionQuickSwitch9()
      ? `${t("Use")} ${shortcutText(shortcuts.sessionQuickSwitch1())} ${t("through")} ${shortcutText(shortcuts.sessionQuickSwitch9())} ${t("to switch pinned sessions")}`
      : undefined,
  t("Run {highlight}/compact{/highlight} to summarize long sessions near context limits"),
  (shortcuts) =>
    `${t("Use")} ${commandText("/export", shortcuts.sessionExport())} ${t("to save the conversation as Markdown")}`,
  ...
  (shortcuts) =>
    `${t("The leader key is")} ${shortcutText(shortcuts.leader())}; ${t("combine with other keys for quick actions")}`,
  ...
  (shortcuts) =>
    shortcuts.messagesPageUp() && shortcuts.messagesPageDown()
      ? `${t("Use")} ${shortcutText(shortcuts.messagesPageUp())}/${shortcutText(shortcuts.messagesPageDown())} ${t("to navigate through conversation history")}`
      : undefined,
  ...
  t("Switch to {highlight}Plan{/highlight} agent for suggestions without making changes"),
  ...
  (shortcuts) => {
    ...
    return `${t("Use")} ${items.map(shortcutText).join(" / ")} ${t("for parent/child sessions")}`
  },
  t("Create {highlight}opencode.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI"),
  ...
  t("Add {highlight}.md{/highlight} files to {highlight}.opencode/commands/{/highlight} for reusable prompts"),
  t("Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input"),
  t("Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})"),
  ...
  t('Set {highlight}"formatter": true{/highlight} to enable built-in formatters'),
  t('Set {highlight}"formatter": false{/highlight} to disable inherited formatters'),
  t("Define custom formatter commands with file extensions in config"),
  t('Set {highlight}"lsp": true{/highlight} to enable built-in LSP code analysis'),
  ...
  t("Create a plugin to prevent OpenCode from reading sensitive files"),
  ...
  t("Run {highlight}opencode serve{/highlight} for headless API access to OpenCode"),
  ...
  t("Use {highlight}{env:VAR_NAME}{/highlight} for environment variables in config"),
  ...
  (shortcuts) =>
    `${t("Use")} ${commandText("/timeline", shortcuts.sessionTimeline())} ${t("to jump to specific messages")}`,
  (shortcuts) => press(shortcuts.messagesToggleConceal(), "to toggle code block visibility in messages"),
  (shortcuts) => `${t("Use")} ${commandText("/status", shortcuts.statusView())} ${t("to see system status info")}`,
  t("Enable {highlight}scroll_acceleration{/highlight} in {highlight}tui.json{/highlight} for smooth scrolling"),
  (shortcuts) =>
    shortcuts.commandList()
      ? `${t("Toggle username display in chat via the command palette")} (${shortcutText(shortcuts.commandList())})`
      : t("Toggle username display in chat via the command palette"),
  t("Run {highlight}docker run -it --rm ghcr.io/anomalyco/opencode{/highlight} in a container"),
  t("Use {highlight}/connect{/highlight} with OpenCode Zen for curated, tested models"),
  ...
  (shortcuts) => `${t("Use")} ${commandText("/help", shortcuts.helpShow())} ${t("to show the help dialog")}`,
